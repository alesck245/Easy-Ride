...
public class Image {
	...
	public void setResizedBitmap(File file, int width, int height) {
		try {
			...
			Bitmap auxBitmap = Bitmap.createBitmap(bitmap, 0, 0, w, h, matrix,
					true);
	...
	private static Bitmap fixMatrix(File file, Bitmap bitmap)
		...
		Matrix matrix = new Matrix();
		boolean fixed = false;
		ExifInterface exif = new ExifInterface(file.getPath()); // Classe para
																// ler tags
																// escritas no
																// JPEG
		int orientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION,
				ExifInterface.ORIENTATION_NORMAL); // Orientação que foi salva a
													// foto
